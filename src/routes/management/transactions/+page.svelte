<script lang="ts">
	...
	$effect(() => {
		const serverValue = data.filters.attendeeName ?? '';
		if (serverValue !== lastSyncedAttendeeName) {
			lastSyncedAttendeeName = serverValue;
			attendeeNameValue = serverValue;
		}
	});
